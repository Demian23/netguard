...
void init_macs();
bool is_valid_mac(std::string& interface, const Device &dev);
...
bool is_valid_machine_params(const Device &dev)
{
    ...
    if(sysctlbyname("hw.model", temp, &size, 0, 0) != 0){
        errors::Sys("sysctlbyname, License::is_valid_machine_params");
    }
...
bool is_valid_mac(std::string& interface, const Device &dev)
{
    ...
    sockaddr_in ip, mask;
    char temp[18] = {};
    if(!host_addr::findownaddr(interface, mac, ip, mask))
       return false; 
